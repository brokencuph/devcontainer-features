...
INSTALL_COMMANDLINE_TOOLS=${INSTALLCOMMANDLINETOOLS}
...
cuda_dev_pkg="cuda-compiler-${CUDA_VERSION/./-} cuda-libraries-dev-${CUDA_VERSION/./-}"
cuda_tools_pkg="cuda-command-line-tools-${CUDA_VERSION/./-}"
...
if [ "$INSTALL_COMMANDLINE_TOOLS" = "true" ]; then
    if ! apt-cache show "$cuda_tools_pkg"; then
        echo "The requested version of CUDA command-line tools is not available: CUDA $CUDA_VERSION"
        exit 1
    fi

    echo "Installing CUDA command-line tools..."
    apt-get install -yq "$cuda_tools_pkg"
fi
